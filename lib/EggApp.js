...
class EggApp {
    constructor() {
        ...
        this.methods = {
            ...
            redirect: (...args) => this.pushRoute(args, '')
        };
        this.router = {
            namespace: prefix => {
                return {
     ...
     */
    initCtrl() {
        const list = fs.readdirSync('app/controller').filter(elem => path.extname(elem) === '.js');
        for (const ctrl of list) {
            const Class = require(path.join(process.cwd(), 'app/controller', ctrl));
